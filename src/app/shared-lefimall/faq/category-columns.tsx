...
export const getColumns = ({
...
}: Columns) => [
  {
    ...
    key: 'id',
    width: 50,
    render: (value: string) => <Text className=' text-base text-center'>{value}</Text>,
  },
 
  {
    title: <HeaderCell title="분류" />,
    dataIndex: 'name',
    key: 'name',
    width: 150,
    render: (value: string) => (
      <Text className=' text-base text-center'>{value}</Text>
    ),
  },
    ...
    render: (_: string, row: any) => (
      <div className="flex items-center justify-end gap-3 pe-4">
        ...
        <Tooltip
          ...
          color="invert"
        >
          <Link href={routes.operation.faqcategoryEdit(row.id)}>
            <ActionIcon
              tag="span"
              size="sm"
              variant="outline"
        ...
        <DeletePopover
          title={`Delete`}
          description={`#${row.id} Are you sure you want to delete?`}
