...
export default function Uploader1({ onSave }: { onSave: (url: string) => void }) {
    ...
    (event: ChangeEvent<HTMLInputElement>) => {
      ...
      if (file) {
        if (file.size / 1024 / 1024 > 50) {
          ...
          //console.log("onChangePicture file=", file);
    ...
    >
      ...
      >
        <div className="space-y-1 mb-2   ">
          {/*
          <h2 className="text-base font-semibold">File Upload</h2>
          */}
          {/*
        ...
        <label
          ...
          className="group relative mt-2 flex h-16 cursor-pointer flex-col items-center justify-center rounded-full border border-gray-300 bg-white shadow-sm transition-all hover:bg-gray-50"
        >
        */}
        ...
        <label
          htmlFor='image-upload'
          className={`${
            dragActive ? '    border-0   border-black' : ''
            
          } 
          relative mt-2 flex h-12 w-12 cursor-pointer flex-col items-center justify-center rounded-full border border-gray-300 bg-white shadow-sm transition-all hover:bg-gray-50`}
